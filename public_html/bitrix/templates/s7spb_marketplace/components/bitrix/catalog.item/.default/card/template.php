...
<div class="product-item">
    ...
    if(is_set($arParams["USER_FAVORITES"])){
        if (in_array($item["ID"], $arParams["USER_FAVORITES"]))
        {
            ?>
            <div class="item-favorite-fill"
                 onclick="toggleFavorite(this, <?=$item["ID"]?>)"
                 title="<?=GetMessage("CATALOG_IZB_IN")?>">
            </div>
            <?
        }else{
            ?>
			...
			switch ($blockName)
			{
				...
				case 'quantity':
					...
					if (!$haveOffers)
					{
						...
						{
                            $MOQ = $actualItem["DISPLAY_PROPERTIES"]["MOQ"]["VALUE"] ? $actualItem["DISPLAY_PROPERTIES"]["MOQ"]["VALUE"] : 1;
							?>
								...
								<div class="product-item-amount">
									...
									<div class="product-item-amount-field-container">
										...
										<input class="product-item-amount-field" id="<?=$itemIds['QUANTITY']?>"
                                               type="number"
                                               data-moq="<?=$MOQ?>"
                                               data-pcs="<?=$actualItem["PROPERTIES"]["Master_CTN_PCS"]["VALUE"]?>"
                                               data-title="<?=Loc::getMessage("CATALOG_WARNING")?>"
                                               data-hint="<?=$actualItem["DISPLAY_PROPERTIES"]["MOQ"]["HINT"]?>"
